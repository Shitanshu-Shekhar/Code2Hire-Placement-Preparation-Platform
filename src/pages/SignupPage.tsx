...
import { Code2, Eye, EyeOff, Mail, Lock, User, ArrowRight, Sparkles } from 'lucide-react'
...
export function SignupPage({ onSignup, onSwitch }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setErrorMsg('')
    setLoading(true)
    setTimeout(() => onSignup(name.trim(), email.trim()), 600)
  }
      ...
      <div className="auth-container">
        <div className="auth-left">
          ...
          <div className="auth-hero-content">
            <div className="auth-pill"><Sparkles size={12} /> START YOUR JOURNEY</div>
            <h1>Your placement<br />prep starts <em>here.</em></h1>
            <p>Join thousands of students preparing smarter with AI-driven study plans, company-specific questions, and real-time contest tracking.</p>
            <div className="auth-features">
              <div className="auth-feature"><span className="auth-feature-dot" />Company-wise curated questions</div>
              <div className="auth-feature"><span className="auth-feature-dot" />AI-powered study plans</div>
              <div className="auth-feature"><span className="auth-feature-dot" />Real-time contest tracking</div>
              <div className="auth-feature"><span className="auth-feature-dot" />Detailed coding analytics</div>
            </div>
          </div>
        </div>
        ...
        <div className="auth-right">
          <form className="auth-form" onSubmit={handleSubmit}>
            ...
            <div className={`auth-field ${focused === 'confirm' ? 'focused' : ''} ${confirm ? 'has-value' : ''} ${!passwordMatch ? 'error' : ''}`}>
              <Lock size={16} className="auth-field-icon" />
              <input
                type="password"
            ...
            {!passwordMatch && <p className="auth-error">Passwords don't match</p>}
            {errorMsg && <p className="auth-error">{errorMsg}</p>}

            <button type="submit" className={`auth-submit ${loading ? 'loading' : ''}`} id="signup-submit">
